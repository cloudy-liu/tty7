...
| `markdown_theme` | string | `"paperglow"` | Independent [Markdown reading theme](/customization/markdown-themes) package ID; its light/dark variant follows the active app mode. |
...
| Key | Type | Default | |
|---|---|---|---|
| `shell` | object | — | `{"program": "fish", "args": ["-l"]}`. Unset uses the platform default. `args` you write are always launched verbatim. They normally turn off [shell integration](/reference/shell-integration); custom `cmd.exe` is the exception when its startup loads Clink (for example, Cmder), because tty7 can integrate it through the environment without changing argv. |
| `custom_shells` | array | `[]` | Extra entries for the new-tab menu: `[{"label": "Ubuntu", "program": "wsl.exe", "args": ["-d", "Ubuntu"]}]`. Launched exactly as written, listed after the detected shells. An entry with arguments normally skips [shell integration](/reference/shell-integration), so it has no prompt marks, working-directory tracking, or command-finished notifications. Custom `cmd.exe` entries that load Clink (including Cmder) receive environment-only integration while keeping every argument unchanged. An entry with no arguments, on a shell tty7 recognizes, is integrated like the detected row beside it. An entry with no `program` is skipped; with no `label` it is named after its program. |
