...
**fish**, **PowerShell**, **CMD / Clink** (including Cmder), and **WSL**. A
custom CMD startup must load Clink for tty7 to integrate it without changing
the configured arguments. Other shells (nushell, elvish, xonsh, and the rest)
run perfectly well in a pane; they simply do not get the prompt layer.
...
CMD reports prompt boundaries and cwd, but not reliable exit codes.
