...
<Frame caption="Placeholder — screenshot: a sidebar of agent sessions across several repos, each with a brand avatar and status symbol">
...
the status symbols, the notifications, the tray icon, and `tty7 wait`. For
agents with a herdr detection manifest, the window also reads the status off
the agent's screen, so Cursor and Antigravity get a status with no hooks at all.
...
<CardGroup cols={3}>
  <Card title="Status and notifications" icon="circle-dot" href="/agents/status">
    Hooks, screen reading, status symbols, the tray icon.
  </Card>
